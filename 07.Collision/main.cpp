...
int main()
{
	...
	*/
	...
	//		core::vector3df(0,-10,0), core::vector3df(0,30,0), 0.0f);
	...
	if (selector)
	{
		scene::ISceneNodeAnimator* anim = new irr::scene::CSceneNodeAnimatorWoWCollisionAnimator(smgr, selector, camera, core::vector3df(30, 50, 30), core::vector3df(0, -10, 0), core::vector3df(0, 30, 0), 0.007f);
		selector->drop(); // As soon as we're done with the selector, drop it.
		camera->addAnimator(anim);
		anim->drop();  // And likewise, drop the animator when we're done referring to it.
	}
